// m9_background_ps.hlsl
// Renders the background tiles for mapper9
...
float4 main(VS_OUTPUT i) : SV_TARGET
{
	...
	// get the mab of name table - divide indices by 8
	name_table_shift = (name_table_shift >> 3) | (name_table_sub_index << 2) | ((name_table_index & 0x1) << 4);
	name_table_sub_index = (name_table_index >> 1) & 0x3;
	name_table_index = name_table_index >> 3;
	uint4 name_msb4 = nametable_msb[name_table_index];
	uint use_alt_pattern = (name_msb4[name_table_sub_index] >> name_table_shift) & 0x1;
